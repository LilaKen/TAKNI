...
from loss.mskw import NuclearWassersteinDiscrepancy as MSKW
from loss.HSL import HypersphereLocalizationLoss as HSL
...
class train_utils(object):
    ...
    def setup(self):
        ...
        elif args.bottleneck and args.bottleneck_capsulenet:
            self.bottleneck_layer = getattr(bottlenecks, 'CapsuleBottleNet')(class_num=Dataset.num_classes)
            self.classifier_layer = nn.Linear(args.bottleneck_num, Dataset.num_classes)
        else:
        ...
        if args.domain_adversarial:
            ...
            if args.adversarial_loss == "CDA" or args.adversarial_loss == "CDA+E" or args.adversarial_loss == "UDAFDF":
                if args.bottleneck:
        ...
        if args.distance_metric:
            ...
            elif args.distance_loss == "LJMMD":
                self.softmax_layer = nn.Softmax(dim=1)
                self.softmax_layer = self.softmax_layer.to(self.device)
                self.distance_loss = DSAN(class_num=Dataset.num_classes)
            elif args.distance_loss == "CORAL":
                self.distance_loss = CORAL
            elif args.distance_loss == 'MSKW':
                self.distance_loss = MSKW
            else:
        ...
        if args.domain_adversarial:
            if args.adversarial_loss == 'DA':
                ...
                self.adversarial_loss = nn.BCELoss()
            elif args.adversarial_loss == "UDAFDF":
                self.softmax_layer_ad = nn.Softmax(dim=1)
                self.softmax_layer_ad = self.softmax_layer_ad.to(self.device)
                self.adversarial_loss = HSL(num_classes=Dataset.num_classes, feature_dim=int(self.model.output_num() / 2))
            else:
                raise Exception("loss not implement")
        else:
                        ...
                        if phase != 'source_train' or epoch < args.middle_epoch:
                            ...
                            if self.distance_loss is not None:
                                if args.distance_loss == 'MK-MMD':
                                    ...
                                    softmax_out = self.softmax_layer(outputs)
                                    distance_loss = self.distance_loss.get_loss(features.narrow(0, 0, labels.size(0)),
                                                                                features.narrow(0, labels.size(0),
                                                                                                inputs.size(
                                                                                                    0) - labels.size(
                                                                                                    0)),
                                                                                labels.narrow(0, 0, labels.size(0)),
                                                                                softmax_out.narrow(0, labels.size(0),
                                                                                                   inputs.size(
                                                                                                       0) - labels.size(
                                                                                                       0)),
                            ...
                            if self.adversarial_loss is not None:
                                if args.adversarial_loss == 'DA':
                                    ...
                                    adversarial_loss = self.adversarial_loss(adversarial_out.squeeze(),
                                                                             adversarial_label)
                                    ...
                                    weight = torch.cat((entropy_source / torch.sum(entropy_source).detach().item(),
                                                        entropy_target / torch.sum(entropy_target).detach().item()),
                                                       dim=0)

                                    adversarial_loss = torch.sum(
                                        weight.view(-1, 1) * self.adversarial_loss(adversarial_out.squeeze(),
                                                                                   adversarial_label)) / torch.sum(
                                        weight).detach().item()
                                    iter_num += 1
                                elif args.adversarial_loss == "UDAFDF":
                                    # 计算 softmax 输出和特征交互
                                    ...
                                    domain_label_target = torch.zeros(inputs.size(0) - labels.size(0)).float().to(
                                        self.device)
                                    adversarial_label = torch.cat((domain_label_source, domain_label_target),
                                                                  dim=0).long()  # 转换为整数类型
                                    adversarial_loss = self.adversarial_loss(adversarial_out.squeeze(),
                                                                             adversarial_label)
                                else:
