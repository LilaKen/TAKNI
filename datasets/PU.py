...
# just Bearings with real damages working condition transfer, if want to use undamaged and artificially damaged,
# should to activate the code
signal_size = 1024
...
def get_files(root, N):
    ...
    for i in range(len(N)):
        ...
        #         path1=os.path.join(root,HBdata[i],HBdata[i],name1+".mat")        #_1----->1 can be replaced by the number between 1 and 20
        ...
        #         path2=os.path.join(root,ADBdata[j], ADBdata[j], name2+".mat")
        ...
        for k in range(len(RDBdata)):
            for w3 in range(1):
                ...
                path3 = os.path.join(root, RDBdata[k], RDBdata[k],  name3 + ".mat")
