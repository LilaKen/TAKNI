...
class MH_GAT(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        print("Min value in x:", torch.min(x))
        print("Max value in x:", torch.max(x))
        if torch.isnan(x).any() or torch.isinf(x).any():
            print("x contains invalid values (NaN or Inf).")
        x = torch.cat((x, in_degrees_f, out_degrees_f), dim=1)

        x = self.conv1(x=x, edge_index=None)
...
def Gen_edge(atrr):
    ...
    edge_index = torch.tensor([[], []],
                              dtype=torch.long)  # 这是一个二维的 PyTorch 张量（tensor），其中包含两个空列表，数据类型为long tensor([], size=(2, 0), dtype=torch.int64)
    ...
    for i in range(indices.shape[0]):  # 遍历从0-9
        ...
        edge_index = torch.cat([edge_index, sub_index],
                               axis=1)  # 将上面的矩阵按列cat for循环结束后得到的是一个2 X 100的矩阵   边索引矩阵 上面一行是源节点 下面一行表示目标节点
