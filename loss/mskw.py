...
def NuclearWassersteinDiscrepancy(source, target, source_features, target_features):
    pre_s = F.softmax(source, dim=1)
    pre_t = F.softmax(target, dim=1)

    feature_loss = JAN(source_features, target_features)
    label_loss = (-torch.norm(pre_t, 'nuc') + torch.norm(pre_s, 'nuc')) / target.size(0)
