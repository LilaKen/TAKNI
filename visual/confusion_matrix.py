...
weights_path = '../checkpoint/informer/CWRU/FFT/informer_features_fft_1d_CWRUFFT_0_to_1_2023_False_CDA+E_True_NWD_adam_step/60-1.0000-best_model.pth'  # 权重文件路径
...
classifier_layer = nn.Linear(256, 10)
...
# Load the datasets
Dataset = getattr(datasets, 'CWRUFFT')
datasets = {}
transfer_task = [[0], [1]]
...
datasets['source_train'], datasets['source_val'], datasets['target_train'], datasets[
    'target_val'] = Dataset('../dataset/CWRU', transfer_task, '0-1').data_split(
...
import matplotlib
matplotlib.use('Agg')  # 使用 'Agg' 后端，适合无界面环境
...
sns.heatmap(
    ...
    annot_kws={"size": 24}  # 👈 关键：设置单元格内数字的字体大小
)


# 添加标题和标签
# plt.title("Confusion Matrix of Classification Model", fontsize=24)
...
# 保存并显示混淆矩阵
plt.savefig("cwru_0-1_2023_confusion_matrix.pdf", format='pdf', bbox_inches='tight')
# plt.show()